...
pub fn compress_into<'a, B: Buffer<'a>>(
    ...
    instances::TEEWORLDS_HUFFMAN.compress(input, buffer)
}

/// Compresses some bytes using the Teeworlds-specific Huffman code.
pub fn compress(input: &[u8]) -> Vec<u8> {
    instances::TEEWORLDS_HUFFMAN.compress_into_vec(input)
}
...
pub fn decompress_into<'a, B: Buffer<'a>>(
    ...
    instances::TEEWORLDS_HUFFMAN.decompress(input, buffer)
}
...
pub fn decompress(input: &[u8]) -> Result<Vec<u8>, InvalidInput> {
    instances::TEEWORLDS_HUFFMAN.decompress_into_vec(input)
}
