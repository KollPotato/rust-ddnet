...
impl From<df::ReaderError> for Error {
    fn from(err: df::ReaderError) -> Error {
        Error::Df(err)
    }
}
...
impl From<io::Error> for Error {
    fn from(err: io::Error) -> Error {
        Error::Df(df::ReaderError::Io(err))
    }
}
...
impl MapReader {
    pub fn open<P: AsRef<Path>>(path: P) -> Result<MapReader, Error> {
        fn inner(path: &Path) -> Result<MapReader, Error> {
            Ok(MapReader::from_datafile(df::Reader::open(path)?))
        }
        inner(path.as_ref())
    }
    pub fn from_datafile(reader: df::Reader) -> Self {
        Self { reader }
    }
