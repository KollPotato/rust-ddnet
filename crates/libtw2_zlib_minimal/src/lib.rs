...
impl Error {
    ...
    pub fn kind(self) -> Result<ErrorKind, ()> {
        Ok(match self.inner {
            libz_sys::Z_MEM_ERROR => ErrorKind::OutOfMemory,
            libz_sys::Z_BUF_ERROR => ErrorKind::OutputBufferTooSmall,
            libz_sys::Z_DATA_ERROR => ErrorKind::InvalidInput,
            _ => return Err(()),
        })
    }
...
pub fn compress_bound(source_len: usize) -> usize {
    (unsafe { libz_sys::compressBound(source_len as c_ulong) }) as usize
}
