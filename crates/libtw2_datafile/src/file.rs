...
use libtw2_common::io::seek_overflow;
use libtw2_common::io::FileExt;
use libtw2_common::io::ReadExt;
...
impl From<format::Error> for ReaderError {
    fn from(err: format::Error) -> ReaderError {
        ReaderError::Df(err)
    }
}
...
trait ResultExt {
    type T;
    fn retrieve(self, error: &mut Option<io::Error>) -> Result<Self::T, ReaderError>;
}

impl<T> ResultExt for Result<T, raw::Error> {
    type T = T;
    fn retrieve(self, error: &mut Option<io::Error>) -> Result<T, ReaderError> {
        self.map_err(|e| match e {
            raw::Error::Df(e) => ReaderError::Df(e),
            raw::Error::Callback => ReaderError::Io(error.take().unwrap()),
        })
    }
}

impl Reader {
    fn new_impl(file: File, check_initial_offset: bool) -> Result<Reader, ReaderError> {
        let mut file = file;
    ...
    pub fn new(file: File) -> Result<Reader, ReaderError> {
        Reader::new_impl(file, true)
    }
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Reader, ReaderError> {
        fn inner(path: &Path) -> Result<Reader, ReaderError> {
            Reader::new_impl(File::open(path)?, false)
        }
        inner(path.as_ref())
    }
    pub fn debug_dump(&mut self) -> Result<(), ReaderError> {
        Ok(self
            .raw
    ...
    pub fn read_data(&mut self, index: usize) -> Result<Vec<u8>, ReaderError> {
        self.raw
    ...
    pub fn data_iter(&mut self) -> DataIter {
        fn map_fn(i: usize, self_: &mut &mut Reader) -> Result<Vec<u8>, ReaderError> {
            self_.read_data(i)
        }
...
pub type DataIter<'a> =
    MapIterator<Result<Vec<u8>, ReaderError>, &'a mut Reader, ops::Range<usize>>;
